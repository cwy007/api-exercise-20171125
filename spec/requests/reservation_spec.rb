...
RSpec.describe "API_V1::Reservations", :type => :request do
  before do
    ...
    @train1 = Train.create!( :number => '0603' )
  ...
  describe "POST /api/v1/reservations" do
    ...
    example 'failed' do
      post '/api/v1/reservations', :params => {
                                     :train_number => @train1.number,
                                     :seat_number => '1A',     # NOTE: error_reason: duplicate
                                     :customer_name => 'zoo',
                                     :customer_phone => '55555555'}
  ...
  describe 'GET /api/v1/reservations' do
    ...
    example 'success' do
      @reservation1 = Reservation.create!( :user_id => @user.id, :train_id => @train1.id,
                                           :seat_number => '1B', :customer_name => 'foo', :customer_phone => '12345678')
      @reservation2 = Reservation.create!( :user_id => @user.id, :train_id => @train1.id,
                                           :seat_number => '1C', :customer_name => 'foo', :customer_phone => '12345678')
      ...
      expect(response.body).to eq( respected_result.to_json )
    end
  end
end
